...
import { IEvent } from "@src/utils/interfaces";
...
export const eventsApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getEventContent: builder.query<any, number>({
      query: (id) => `/events/content/${+id - 1}`,
    }),
  }),
});

export const { useGetEventsQuery, useGetEventQuery, useGetEventContentQuery } =
  eventsApi;
